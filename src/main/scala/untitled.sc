...
abstract class ZStream[-R, +E, +O](val process: ZManaged[R, Nothing, ZIO[R, Option[E], Chunk[O]]]) { self =>
  ...
  final def broadCastAsTuple[O1, O2](maximumLag: Int, f1: O => O1, f2: O => O2, f3: (O1, O2) => Either[O1, O2]): ZManaged[R, Nothing, (ZStream[Any, E, O1], ZStream[Any, E, O2])] =
    self.broadcast(2, maximumLag).map { list => (list.head.map(f1), list.tail.head.map(f2)) }
      .map(a => a._1)
      ...
      for {
        ...
        pull = done.get.flatMap {
          if (_) Pull.end
            ...
            queue.take.flatMap(_.foldM(done.set(true) *> Pull.end, Pull.halt, Pull.emit))
        }
      } yield pull
    }
        ...
        pull = {
          ...
          def failover(cause: Cause[Option[E]]) =
            Cause.sequenceCauseOption(cause) match {
              ...
              case Some(cause) =>
                closeCurrent(cause) *> open(f(cause))(Other).flatten
            }
  ...
  final def collectM[R1 <: R, E1 >: E, O1](pf: PartialFunction[O, ZIO[R1, E1, O1]]): ZStream[R1, E1, O1] =
    ZStream {
      for {
        ...
        pfSome = pf.andThen(_.bimap(Some(_), Chunk.single))
        pull = {
  ...
  final def collectWhileM[R1 <: R, E1 >: E, O2](pf: PartialFunction[O, ZIO[R1, E1, O2]]): ZStream[R1, E1, O2] =
    ZStream {
      for {
        ...
        pfIO  = pf.andThen(_.bimap(Some(_), Chunk.single))
        pull = done.get.flatMap {
          if (_) Pull.end
          else
  ...
  final def distributedWith[E1 >: E](
    ...
    Promise.make[Nothing, O => UIO[UniqueKey => Boolean]].toManaged_.flatMap { prom =>
      distributedWithDynamic(maximumLag, (o: O) => prom.await.flatMap(_(o)), _ => ZIO.unit).flatMap { next =>
        ZIO.foreach(Range(0, n))(id => next.map { case (key, queue) => ((key -> id), queue) }).flatMap { entries =>
          val (mappings, queues) =
  ...
  def partitionTuple(buffer: Int = 16): ZManaged[R, E, (ZStream[Any, E, O], ZStream[Any, E, O])] =
    self.distributedWith(2, buffer,)
  /**
